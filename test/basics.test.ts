...
import { bufToBn, hexToBuf, toUint8Array } from "../src/utils"

test("testing numbers in range from seed", () => {
    const seed = "xyz"
    ...
    expect(numbersInRange.getNextNumber()).toBe(a1)
    expect(numbersInRange.getNextNumber()).toBe(a4)
})

test("testing kdfCounterMode", () => {
    const seed = "kdk"
    const label = "label"
    const context = "context"
    const byteLength = 65
    const kdf = kdfCounterMode(byteLength, toUint8Array(seed), label, context)
