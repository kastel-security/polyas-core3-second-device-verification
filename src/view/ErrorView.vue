...
<template>
    ...
    <div class="cause">
        <h3 v-if="errorType==ErrorType.EXTERN">{{ extractTextFromJson(text.error.extern, language) }}</h3>
        ...
        <h3 v-if="errorType==ErrorType.SDPP">{{ extractTextFromJson(text.error.sdpp, language) }}</h3>
        <h3 v-if="errorType==ErrorType.ZKP_INV">{{ extractTextFromJson(text.error.zkp_inv, language) }}</h3>
