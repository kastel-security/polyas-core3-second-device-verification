 ...
 * @returns
 */
function decodeMultiPlaintext (multiPlaintext: Uint8Array): Uint8Array {
...
class NumbersInRangeFromSeed {
  ...
  public async getNextNumber (): Promise<bigint> {
    ...
    const bytes = await kdfCounterMode(byteCount, new Uint8Array([...this.seed, ...toUint8Array(this.i, 4)]), 'generator', 'Polyas')
 ...
 * @returns
 */
async function kdfCounterMode (length: number, seed: Uint8Array, label: string, context: string): Promise<Uint8Array> {
  const blockCount = Math.ceil(length / 64)
