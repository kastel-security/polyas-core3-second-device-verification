 ...
 * @returns
 */
async function checkSecondDeviceParameters(parametersJson) {
 ...
 */
async function generateComKey(ballotNorm, comSeed) {
 ...
 * @returns comKey as Uint8Array
 */
async function generateComKey2(ballotNorm, comSeed) {
    const comSeedBytes = new Array();
 ...
 * @returns
 */
async function aesDecrypt(c, comKey) {
 ...
 */
async function decrytQRCode(c, initialMessage) {
 ...
 * @returns
 */
async function checkZKP(initialMessage, finalMessage, proof, randomCoinSeed) {
 ...
 * @returns
 */
async function decryptBallot(initialMessage, randomCoinSeed) {
    const cipherLength = initialMessage.ballot.encryptedChoice.ciphertexts.length;
    const q = BigInt('0x' + constants.q);
 ...
 * @returns
 */
async function generateReceiptText(loginResponse) {
    const electionId = loginResponse.electionId;
    const voterId = loginResponse.ballotVoterId;
    const fingerprint = await computeFingerprint(loginResponse);
    const shortenedFingerprint = fingerprint.substring(0, 10);
    const signature = loginResponse.initialMessageDecoded.signatureHex;
    const string = new Array(`Project ID: ${electionId}`, `Voter Id: ${voterId}`, `Ballot Fingerprint: ${shortenedFingerprint}`, `-----BEGIN FINGERPRINT-----${fingerprint}`, '-----END FINGERPRINT-----', '-----BEGIN SIGNATURE-----', `${signature}`, '-----END SIGNATURE-----');
    return string;
}
