...
function put (bytes: Uint8Array, input: string | Uint8Array | number | bigint, length?: number): Uint8Array {
  const bytesOfInput: Uint8Array = toUint8Array(input, length)
  return new Uint8Array([...bytes, ...bytesOfInput])
}

function putWithLength (bytes: Uint8Array, input: string | Uint8Array | number | bigint): Uint8Array {
  const bytesOfInput: Uint8Array = toUint8Array(input)
 ...
 * @returns
 */
function getBallotAsNormalizedBytestring (ballot: Ballot): Uint8Array {
...
async function checkSignature (response: SecondDeviceLoginResponse): Promise<boolean> {
  const publicKeyHex: string = response.initialMessageDecoded.secondDeviceParameterDecoded.verificationKey
